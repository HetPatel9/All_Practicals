...
let flag:interfaces.flag = {
    func: false,
    trigoBox: false,
    trigoFunc: true,
    isDegree: false,
}
let display:interfaces.displayArray = [];
let memory = 0;
let hCounter = -1;
function buttonFunctionality(btnTxt:(string| number)): void {
    const trigoBox = document.querySelector('.trigonometry') as HTMLButtonElement;
    const trigoFunc= document.querySelector('.rg1') as HTMLButtonElement;
    const inverseTrigoFunc= document.querySelector('.rg2') as HTMLButtonElement;
    switch (btnTxt) {
        case '1':
            display.push(1);
            break;
        ...
        case 'log':
            ...
            break;
        case '+/-':
            let change:interfaces.displayArray = [];
            for (let i = display.length - 1; i > -1; i--) {
                if (isNaN(display[i] as number)) {
                    break;
                }
                else {
        ...
        case 'Ceil':
            let ceil = Math.ceil(eval(display.join('')));
            display = [];
            display.push(ceil);
            break;
        case 'Floor':
            let floor = Math.floor(eval(display.join('')));
            display = [];
            display.push(floor);
            break;
        case 'Round':
            let round = Math.round(eval(display.join('')));
            display = [];
            display.push(round);
            break;
        case 'ln':
        ...
        case 'RAD':
            flag.isDegree = true;
            let RtoD = document.querySelector('.DtoR') as HTMLButtonElement;
            RtoD.innerHTML = 'DEG';
            
            break;
        case 'DEG':
            flag.isDegree = false;
            let DtoR = document.querySelector('.DtoR') as HTMLButtonElement;
            DtoR.innerHTML = 'RAD';
            break;
        case 'sin':
            display.push('sin(');
        ...
        case 'tanh-1':
            ...
            break;
        case 'Function':
            const row1 = document.querySelector('.rg3') as HTMLButtonElement;
            if (flag.func == false) {
        ...
        case 'Inv':
            ...
            break;
        case 'F-E':
            let tempAns = eval(display.join('')).toExponential();
            display = [];
            display.push(tempAns);
            break;
        case '=':
            let ans = calculation(display);
            display = [];
            display.push(ans);
            break;
        case 'MC':
    ...
    let ans = document.querySelector('input') as HTMLInputElement;
    ans.value = display.join('');
}

let sArray:interfaces.displayArray = [];
let calArray: interfaces.displayArray= [];
let historyArray:number[] = [];
function calculation(temp:interfaces.displayArray) {
    sArray = temp;
...
function factNum(index:number){

    let num = [];

        if (isNaN(sArray[index - 1] as number) && sArray[index - 1] != ')') {
            return new Error('ERror');
        }
        else if (sArray[index - 1] != ')') {
            for (let i = index - 1; i >= 0; i--) {
                if (!isNaN(sArray[i] as number)) {
...
function fact(n:number): string|number{
    let ans: string|number = 1;
    if (n == 0) {
        return 1;
    }
    else if (n > 0) {
